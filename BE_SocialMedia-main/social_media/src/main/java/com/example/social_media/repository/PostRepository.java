...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query("SELECT p FROM Post p WHERE LOWER(p.content) LIKE LOWER(CONCAT('%', :keyword, '%')) AND p.status <> :status")
    List<Post> searchByContentAndStatusNotRejected(@Param("keyword") String keyword, @Param("status") Post.Status status);
